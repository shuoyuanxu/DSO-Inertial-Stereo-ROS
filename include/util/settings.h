...
// mono-VI scale-divergence reset bounds (T_WD scale outside [low,high] -> full reset).
// Upstream hardcoded 0.1/10; widen for low-excitation platforms where scale converges slowly.
extern double setting_scaleResetLow;
extern double setting_scaleResetHigh;

// Range-gate for static-stereo depth: points farther than this (metres) are
// dropped from stereo initialization, because a short baseline cannot
// triangulate them reliably and their noisy metric depth distorts the map.
// Default huge = off (upstream behaviour). Set ~10-15 m for a short-baseline rig.
extern double setting_stereoMaxDepth;

// carry bias/scale estimates across re-initializations (OKVIS-style: a reset should
// not restart physical sensor states from scratch). Updated while tracking, consumed
// by initFirstFrame_imu on the next (re-)initialization.
extern bool setting_carryStateOverReset;
extern Vec3 carried_bias_g;
extern Vec3 carried_bias_a;
extern double carried_scale;
extern bool have_carried_state;
