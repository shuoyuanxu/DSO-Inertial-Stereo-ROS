...
// 		  T_WD = T_WD*change;
		  ...
		  if(!state_twd.allFinite()){
		    // a non-finite linear-solve result would poison T_WD and crash inside
		    // Sophus (ScaleNotPositive on a NaN quaternion) — reset cleanly instead
		    printf("TWD STATE NON-FINITE -> reset\n");
		    state_twd.setZero();
		    initFailed = true;
		    first_track_flag = false;
		    return false;
		  }
		  if(std::exp(state_twd[6])<setting_scaleResetLow||std::exp(state_twd[6])>setting_scaleResetHigh){
		    // scale left its trust region: RESET cleanly. (Clamping-and-continuing
		    // was tried and REGRESSED badly — a diverging scale just masks an
		    // exploding pose optimization, producing NaN fly-away. The clean reset
		    // is what keeps tightly-coupled VI bounded to ~1-2 m on this data.)
		    printf("SCALE OUT OF TRUST REGION (%.4f) -> reset\n", std::exp(state_twd[6]));
		    state_twd.setZero();
		    initFailed = true;
		    first_track_flag = false;
		    return false;
