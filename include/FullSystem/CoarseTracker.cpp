...
// 	LOG(INFO)<<std::fixed<<std::setprecision(9)<<"time_start: "<<time_start<<" time_end: "<<time_start<<" dt: "<<time_end - time_start;
	if(imu_use_flag && imu_time_stamp.size() > 1){   // vision-only mode runs without any IMU data
	...
	std::vector<double> imu_track_w(5,0);   // indices 0..4 are written below; sizing by coarsestLvl overflowed the heap when pyrLevelsUsed<6
	...
	for(int lvl=coarsestLvl; lvl>=0; lvl--)
	{
		...
		if(imu_use_flag && imu_track_flag && lvl<=0){
