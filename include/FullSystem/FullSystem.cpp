...
void FullSystem::addActiveFrame( ImageAndExposure* image, ImageAndExposure* image_right, int id )
{
	...
	if(!T_WD.matrix().allFinite()){
	    printf("T_WD NON-FINITE -> reset\n");
	    T_WD = Sim3();
	    T_WD_l = T_WD;
	    T_WD_l_half = T_WD;
	    state_twd.setZero();
	...
	// upstream assumed baseline-pinned scale (bounds 0.6-2); with weak stereo
	// (far structure vs short baseline) the IMU may need much larger corrections
	if(use_stereo&&(T_WD.scale()>setting_scaleResetHigh||T_WD.scale()<setting_scaleResetLow)){
	    printf("STEREO SCALE OUT OF BOUNDS (%.3f) -> reset\n", T_WD.scale());
	    initFailed = true;
	    first_track_flag = false;
	}
// 	LOG(INFO)<<"allKeyFramesHistory.size(): "<<allKeyFramesHistory.size();
	if(use_stereo==false&&(T_WD.scale()<setting_scaleResetLow||T_WD.scale()>setting_scaleResetHigh)){
	    printf("SCALE OUT OF BOUNDS (%.3f) -> reset\n", T_WD.scale());
	    initFailed = true;
	    first_track_flag = false;
	}
// 	if(use_stereo==false){
...
// 	}
	...
	if(allFrameHistory.size()>0){
	    ...
	    // keep latest physical-state estimates for carry-over across resets
	    if(setting_carryStateOverReset && imu_use_flag && imu_track_ready){
	        carried_bias_g = fh->bias_g;
	        carried_bias_a = fh->bias_a;
	        carried_scale = T_WD.scale();
...
void FullSystem::initFirstFrame_imu(FrameHessian* fh){
	SE3 T_wc;   // identity unless gravity-aligned below
	T_WR_align = SE3();

	if(imu_use_flag && (int)m_acc.size() >= 45)
	{
	int index = -1;
	...
	T_wc = SE3(R_wc,Vec3::Zero());
	if(gt_path.size() > 0)
	    T_WR_align = T_wc * T_BC.inverse()*gt_pose[index2].inverse();
	}
	}

	// OKVIS-style: seed physical sensor states from the previous run instead of zero
	if(setting_carryStateOverReset && have_carried_state){
	    fh->bias_g = fh->shell->bias_g = carried_bias_g;
	    fh->bias_a = fh->shell->bias_a = carried_bias_a;
	    printf("CARRY STATE: bias_g [%.4f %.4f %.4f] bias_a [%.4f %.4f %.4f] scale %.3f\n",
	           carried_bias_g[0], carried_bias_g[1], carried_bias_g[2],
	           carried_bias_a[0], carried_bias_a[1], carried_bias_a[2], carried_scale);
	}
...
// 	LOG(INFO)<<"first pose: \n"<<T_wc.matrix();
	...
	// seed scale from the previous run when carrying state: the mono initializer
	// normalizes mean inverse depth, so in the same environment the fresh visual
	// scale is statistically close to the previous one (heuristic, not exact).
	double s0 = (setting_carryStateOverReset && have_carried_state) ? carried_scale : 1.0;
	T_WD = Sim3(RxSO3(s0,R_wd),Vec3::Zero());
	T_WD_l = T_WD;
	T_WD_l_half = T_WD;
	state_twd.setZero();
}
...
void FullSystem::deliverTrackedFrame(FrameHessian* fh, FrameHessian* fh_right, bool needKF)
{
	...
	if(linearizeOperation)
	{
		...
		else if(!disableAllDisplay) handleKey( IOWrap::waitKey(1) );   // don't pump the GUI event loop when display is disabled
	...
	else
	{
		...
		unmappedTrackedFrames_right.push_back(fh_right);   // keep right queue in sync (was missing: mappingLoop pops both)
	...
	while(runMapping)
	{
		...
		if(unmappedTrackedFrames.size() > 0) // if there are other frames to tracke, do that first.
		{
			...
			if(needToKetchupMapping && unmappedTrackedFrames.size() > 0)
			{
				FrameHessian* fh = unmappedTrackedFrames.front();
				unmappedTrackedFrames.pop_front();
				FrameHessian* fh_right_skip = unmappedTrackedFrames_right.front();   // pop right queue too, else queues desync
				unmappedTrackedFrames_right.pop_front();
				{
					boost::unique_lock<boost::mutex> crlock(shellPoseMutex);
...
// 	LOG(INFO)<<"rmse: "<<rmse;
	...
	{
		printf("INIT GATE: KF %d, rmse %.2f (limits: 20/13/9 x slack %.1f)\n",
		       (int)allKeyFramesHistory.size(), rmse, benchmark_initializerSlackFactor);
...
// 	firstFrame->frame_right->frameID = allKeyFramesHistory.size();
	...
	if(use_stereo){
	    for(int i=0;i<coarseInitializer->numPoints[0];i++)
	    {
		    ...
		    ImmaturePointStatus initStereoStatus = pt->traceStereo(firstFrameRight, &Hcalib, 1);
		    ...
		    // drop points the stereo range-gate rejected (too far for this baseline):
		    // their noisy metric depth would distort the reconstruction
		    if(initStereoStatus != ImmaturePointStatus::IPS_GOOD
